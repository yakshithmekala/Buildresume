...
export const ExecutiveATSTemplate: React.FC<TemplateProps> = ({ data }) => {
  ...
  const sectionOrder: ResumeSectionId[] = data.sectionOrder || [
  ...
  const renderSkills = () => skillCategories.length > 0 ? (
  ...
  const renderAchievements = () => achievements.length > 0 ? (
    <section key="achievements" className="resume-section mb-6">
      <h2 className="text-sm font-bold uppercase tracking-wider text-slate-900 border-b border-slate-300 pb-1 mb-2">
        Key Achievements
      </h2>
  ...
  const renderSectionMap: Record<ResumeSectionId, () => React.ReactNode> = {
  ...
  return (
    <div className="resume-container bg-white text-slate-900 font-sans p-8 md:p-12 max-w-[850px] mx-auto shadow-2xl rounded-sm print:p-6 print:shadow-none print:max-w-full">
      ...
      {sectionOrder.map((secId) => renderSectionMap[secId] ? renderSectionMap[secId]() : null)}
    </div>
  );
};
